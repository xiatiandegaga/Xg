...
namespace Cloud.Caching
{
    public class CsRedisCache : ICache
    {
        ...
        public CsRedisCache(IConfiguration configuration)
        {
            _random = new Random();
            ...
            _redisClient = new CSRedisClient(_configuration["ConnectionStrings:Redis"]);
        }
        ...
        public bool Set(string cacheKey, object value, CachingExpireType cachingExpirationType, bool isAddRandom = true)
        {
            return Set(cacheKey, value, cachingExpireTypeDictionary[cachingExpirationType], isAddRandom);
        }
        ...
        /// <param name="cachingExpirationType">缓存期限类型</param>
        /// <param name="isAddRandom">是否增加缓存随机时间</param>
        public async Task<bool> SetAsync(string cacheKey, object value, CachingExpireType cachingExpirationType, bool isAddRandom = true)
        {
            return await SetAsync(cacheKey, value, cachingExpireTypeDictionary[cachingExpirationType], isAddRandom);
        }
        ...
        /// <param name="isAddRandom">是否增加缓存随机时间</param>
        /// <returns></returns>
        private bool Set(string key, object value, TimeSpan timeSpan, bool isAddRandom = true)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
            ...
            if (isAddRandom)
                return _redisClient.Set(key, JsonUtility.SerializeByte(value), (int)timeSpan.Add(new TimeSpan(0, 0, _random.Next(1, 600))).TotalSeconds);
            else
                return _redisClient.Set(key, JsonUtility.SerializeByte(value), (int)timeSpan.TotalSeconds);
        }
        ...
        /// <param name="isAddRandom">是否增加缓存随机时间</param>
        /// <returns></returns>
        private async Task<bool> SetAsync(string key, object value, TimeSpan timeSpan, bool isAddRandom = true)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
            ...
            if (isAddRandom)
                return await _redisClient.SetAsync(key, JsonUtility.SerializeByte(value), (int)timeSpan.Add(new TimeSpan(0, 0, _random.Next(1, 600))).TotalSeconds);
            else
                return await _redisClient.SetAsync(key, JsonUtility.SerializeByte(value), (int)timeSpan.TotalSeconds);
        }
        ...
        public bool SetNx(string key, object value, CachingExpireType cachingExpirationType, bool isAddRandom = true)
        {
            ...
            if (isAddRandom)
                return _redisClient.Set(key, JsonUtility.SerializeByte(value), (int)timeSpan.Add(new TimeSpan(0, 0, _random.Next(1, 600))).TotalSeconds, RedisExistence.Nx);
            else
                return _redisClient.Set(key, JsonUtility.SerializeByte(value), (int)timeSpan.TotalSeconds, RedisExistence.Nx);
        }
        ...
        public async Task<bool> SetNxAsync(string key, object value, CachingExpireType cachingExpirationType, bool isAddRandom = true)
        {
            ...
            var timeSpan = cachingExpireTypeDictionary[cachingExpirationType];
            if (isAddRandom)
                return await _redisClient.SetAsync(key, JsonUtility.SerializeByte(value), (int)timeSpan.Add(new TimeSpan(0, 0, _random.Next(1, 600))).TotalSeconds, RedisExistence.Nx);
            else
                return await _redisClient.SetAsync(key, JsonUtility.SerializeByte(value), (int)timeSpan.TotalSeconds, RedisExistence.Nx);
        }
        ...
        {
            var timeSpan = cachingExpireTypeDictionary[cachingExpirationType];
            return await _redisClient.ExpireAsync(key, timeSpan);
        }

        public CSRedisClientLock Lock(string name, int timeoutSeconds, bool autoDelay = true) => _redisClient.Lock(name, timeoutSeconds, autoDelay);
        ...
        /// <param name="isAddRandom">是否增加缓存随机时间</param>
        /// <returns></returns>
        public bool SetFreeTime(string key, object value, TimeSpan timeSpan, bool isAddRandom = true)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
        ...
        /// <param name="isAddRandom">是否增加缓存随机时间</param>
        /// <returns></returns>
        public async Task<bool> SetFreeTimeAsync(string key, object value, TimeSpan timeSpan, bool isAddRandom = true)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<bool> ZAddByTsAsync(string key, long value)
        {
            (decimal, long) scoreMembers =(TimeExtension.CurrentTimeMillis(), value);
            var ret = await _redisClient.ZAddAsync(key, scoreMembers);
            if (ret == 1)
                return true;
            return false;
        }
        ...
        /// <returns></returns>
        public async Task<bool> ZAddBySortAsync(string key, decimal score, long value)
        {
            (decimal, long) scoreMembers = (score, value);
            var ret = await _redisClient.ZAddAsync(key, scoreMembers);
            if (ret == 1)
                return true;
            return false;
        ...
        /// <returns></returns>
        public async Task<decimal> ZScoreAsync(string key,long member)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<bool> ZRemAsync(string key, long value)
        {
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<long[]> ZRevRangeAsync(string key,long start=0,long stop=-1)
        {
            return await _redisClient.ZRevRangeAsync<long>(key,start,stop);
        }
        ...
        /// <param name="value"></param>
        /// <returns></returns>
        public async Task<long[]> ZRangeAsync(string key, long start = 0, long stop = -1)
        {
            return await _redisClient.ZRangeAsync<long>(key, start, stop);
        }
        ...
        public void ZRemAll(Dictionary<string, long> dicKeyValues,int length=1000)
        {
            if(length<=0)
            {
            ...
            {
                var pipe = _redisClient.StartPipe();
                foreach (KeyValuePair<string, long> kvp in dicKeyValues)
                {
            ...
            else
            {
                ...
                {
                    var pipe = _redisClient.StartPipe();
                    foreach (KeyValuePair<string, long> kvp in x)
                    {
                        pipe.ZRem(kvp.Key, kvp.Value);
                    }
                    pipe.EndPipe();
