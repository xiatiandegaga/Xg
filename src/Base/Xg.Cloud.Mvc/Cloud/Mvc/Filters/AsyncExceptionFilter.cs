...
namespace Cloud.MvcBuilder.Filters
{
    public class AsyncExceptionFilter : IAsyncExceptionFilter
    {
        ...
        public Task OnExceptionAsync(ExceptionContext filterContext)
        {
            ...
            if (exception.GetType() == typeof(MyException))
            {
                ...
                filterContext.Result = new ObjectResult(new AjaxResponse() { Code = 0, Msg = exception.Message });
            ...
            {
                filterContext.Result = new ObjectResult(new AjaxResponse() { Code = 0, Msg = "发生异常，请联系管理员！" });
                filterContext.ExceptionHandled = true;
                return Task.CompletedTask;
            }
            filterContext.ExceptionHandled = true;
