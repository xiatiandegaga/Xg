...
namespace Cloud.Extensions
{
    ...
    {
        public static void AddDddBasicService(this IServiceCollection services, IConfiguration configuration)
        {
            if (services == null)
           ...
           var lstDomainService = new List<TypeInfo>();
            ...
            var mappingAppTypes = assembly.GetTypes()
                  .Where(x => x.GetInterfaces().Any(y => y == mappingAppInterface)).ToList();
            ...
            var mappingServiceTypes = assembly.GetTypes()
                  .Where(x => x.GetInterfaces().Any(y => y == mappingServiceInterface)).ToList();
            if (mappingServiceTypes != null)
            {
                mappingServiceTypes.ForEach(t =>
                {
                    services.AddScoped(t.GetTypeInfo().ImplementedInterfaces.First(), t);
                });
            }
        }
    }
